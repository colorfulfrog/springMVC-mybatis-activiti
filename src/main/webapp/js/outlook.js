$(function() {
	...
	tabContextMenu(); // 生成Tab右键菜单
	tabMenuEvent(); // 绑定Tab右键菜单事件
	...
	console.log("=======================" + firstMenuName);
	if(firstMenuName){
		addNav(_menus[firstMenuName]); // 添加导航菜单
		InitLeftMenu();
	}
});
...
//清除左侧导航菜单
function Clearnav() {
	var pp = $('#wnav').accordion('panels');//获取所有面板
...
function addNav(data) {
	console.log("=======================左侧菜单JSON：" + JSON.stringify(data));
...
function tabMenuEvent() {
	...
	$('#mm-tabupdate').click(function() {
		...
		console.log("=================refresh current Tab url:" + url);
		$('#tabs').tabs('update', {
			tab : currTab,
			options : {
				content : createFrame(url)
